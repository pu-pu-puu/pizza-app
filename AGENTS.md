...
- `prisma/prisma-client.ts` wraps every query with a fetch timeout (`NEON_FETCH_TIMEOUT_MS`, default 4000ms) and a retry-on-transient-error layer (`ECONNRESET`, `ETIMEDOUT`, `EAI_AGAIN`, `fetch failed`, `AbortError`, etc.) — do not bypass it by importing `PrismaClient` directly.
