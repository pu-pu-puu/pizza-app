...
describe('POST /api/cart', () => {
  ...
  it('returns 404 when the requested productItem does not exist', async () => {
    ...
    const res = await POST(buildCartPost({ productItemId: 999 }, 'tok-existing'));
  ...
  it('returns 409 when the underlying product is inactive', async () => {
    ...
    const res = await POST(buildCartPost({ productItemId: 11 }, 'tok-existing'));
  ...
  it('increments quantity when the same productItem (with matching ingredients) is already in the cart', async () => {
    ...
    vi.mocked(prisma.cartItem.findFirst).mockResolvedValue({
      ...
      productItemId: 11,
      quantity: 1,
    } as unknown as Awaited<ReturnType<typeof prisma.cartItem.findFirst>>);
